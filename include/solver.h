...
constexpr double TOWNSEND = 1e-21;                                    // Td per V/m
constexpr double KB = 1.380649e-23;                                   // J/K
constexpr double ELECTRONVOLT = 1.602176634e-19;                      // Joules
constexpr double EV2K = 1.16045052e4;                                 // Kelvin per eV
...
public:
    ...
    void set_grid() {
        std::cout << "[Alert] Using default grid settings: \"QuadraticGrid\", (0.0, 100.0, 100) \n";
        set_grid("QuadraticGrid", 0.0, 100.0, 100);
    }
    ...
    // TODO: May need to add a destructor if there's any cleanup required.
    ...
    Eigen::VectorXd converge(const Eigen::VectorXd& F0, int maxn = 100, double rtol = 1e-5, double delta0 = 1e14, double m = 2.0, bool full = false);
    int nIter;

    double rate(const Eigen::VectorXd& F0, Process* k, bool weighted = false); //Rate coefficient of process k [m^3/s]
...
private:
    ...
    std::unordered_map<std::string, std::unique_ptr<Target>> m_targetsMap;
    ...
    double EN;                // Reduced electric field [V/m]
    double kT;                // Gas temperature in [eV]
    Eigen::VectorXd density;  // Gas density composition
    Eigen::VectorXd benergy;  // Cell boundary values at i - 1/2
    Eigen::VectorXd cenergy;  // Cell centers
    Eigen::VectorXd denergy;  // deltas
    ...
    std::pair<Eigen::SparseMatrix<double>, Eigen::SparseMatrix<double>> _linsystem(const Eigen::VectorXd& F0);
